...
vector <double> enter(int number_count)
{
 ...
 vector<size_t> make_histogram(vector<double>numbers, size_t bin_count)
{
    ...
    {
        bin=0;
        bin = (size_t)((number - min) / (max - min) * bin_count);
        if (bin == bin_count)
        {
            bin--;
        }
...
void show_gistogram(vector<size_t> bins)
{   size_t bin;
...
cout<< "<rect x='"<< x <<"' y='"<< y << "'  width='" << width << "'  height='" << height<<"'"<<" stroke= '"<<stroke<< "' text= '"<<fill<<"' "<< " />\n";
}
void
...
int main() {
    ...
    cin >> number_count;
   const vector<double> numbers = enter(number_count);
